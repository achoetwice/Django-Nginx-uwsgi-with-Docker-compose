...
urlpatterns = [
    ...
    url(r'^premium_url_pay/(?P<service_customer_id>[\w\x2D]+)/(?P<contract_type>[\w\x2D]+)/$', views.Url_PremiumCustomerPaynow.as_view()),
    ...
    url(r'^fastlaunch_url_pay/(?P<charge_type>[\w]+)/(?P<fastlaunch_no>[\w\x2D]+)/(?P<email>[\w\x2D\@\.]+)/$', views.Url_FastLaunch_Paynow.as_view()),
