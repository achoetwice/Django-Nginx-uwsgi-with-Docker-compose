...
def StoreTempInfo(email, first_name, last_name, customer_mobile, learners, schedule_id, auto_create_account):
    New_temp_info = GuestTemporaryInfo(email = email, first_name = first_name, last_name = last_name, mobile = customer_mobile, \
    learners = learners, schedule_id = schedule_id, auto_create_account = auto_create_account)
    New_temp_info.save()
...
def GetClassInfo(schedule_id):
    ...
    try:
        ...
        return APIHandler("schedule not found")
...
def ECPAY(schedule_id, learners, guest_id, temp_id):
    ...
    order_params = {
        ...
        'ClientBackURL': public_url + '/payment/test/',
        'ItemURL': 'https://www.ecpay.com.tw/item_url.php',
        'Remark': '交易備註',
        'ChooseSubPayment': '',
        'OrderResultURL': public_url + '/payment/show_result/',
        'NeedExtraPaidInfo': 'Y',
        'DeviceSource': '',
        'IgnorePayment': '',
        'PlatformID': '',
...
def Update_Vacancy(schedule_id, learner_count):
    ...
    try:
        ...
        return APIHandler.fail('schedule not found')
    vacancy = schedule.vacancy
    if (vacancy - learner_count) <0:
        return APIHandler.fail('No vacancy')
...
@transaction.atomic
def Upate_Transaction(credict_return_data, schedule_id, learners, class_info):
    ...
    learners_count = len(learners)
    temp_id = eval(credict_return_data['CustomField3'])['t_id']
    ...
    data['guest_id'] = eval(credict_return_data['CustomField2'])['g_id']
